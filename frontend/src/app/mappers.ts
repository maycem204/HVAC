...
export function mapTechnician(row: any): Technician {
  return { id: row.id ?? row.user_id, name: row.name, specialty: row.specialty ?? (row.specializations ?? []).slice(0, 2).join(" & "), specializations: row.specializations ?? [], rating: Number(row.rating ?? 0), reviews: Number(row.reviews_count ?? row.reviews ?? 0), distanceKm: row.distance_km == null ? null : Number(row.distance_km ?? row.distanceKm), available: !!row.available, price: row.price_label ?? row.price ?? "Sur devis", response: row.response_time ?? row.response ?? "—", tags: row.tags ?? [], avatar: row.avatar ?? row.name?.split(" ").map((name: string) => name[0]).join("").slice(0, 2).toUpperCase(), color: row.color ?? "bg-blue-500", lat: Number(row.lat ?? 0), lng: Number(row.lng ?? 0), canRate: Boolean(row.can_rate ?? row.canRate), myRating: row.my_rating == null ? undefined : Number(row.my_rating), myRatingComment: row.my_rating_comment ?? undefined };
}
