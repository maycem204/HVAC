  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50/40 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <h2 className="text-xl font-bold text-foreground mb-2" style={{ fontFamily:"Onest,sans-serif" }}>{i18n.t(isClient?"location.findNearbyTechnicians":"location.defineServiceArea")}</h2>
        <p className="text-sm text-muted-foreground mb-6">{i18n.t(isClient?"location.clientExplanation":"location.technicianExplanation")}</p>
        {state==="ask" && (
        ...
        {state==="denied" && (
          <div className="space-y-3">
            <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-lg border border-amber-100 text-xs text-amber-700"><AlertCircle className="w-4 h-4 shrink-0 mt-0.5"/>{geoError || i18n.t("interface.gps.access.denied.enter.your.city.manually")}</div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-16 text-center">
        ...
        <div className="grid md:grid-cols-2 gap-5 max-w-2xl w-full">
          {[{r:"client" as Role,t:i18n.t("landing.clientRole"),d:i18n.t("landing.clientRoleDescription"),Icon:User,c:"blue"},{r:"technician" as Role,t:i18n.t("landing.technicianRole"),d:i18n.t("landing.technicianRoleDescription"),Icon:Wrench,c:"emerald"}].map(({r,t,d,Icon,c})=>(
...
export function AuthForm({ role, onBack, onLogin }: { role: Role; onBack: () => void; onLogin: (u: AppUser) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err.response?.data?.error ?? i18n.t("interface.an.error.occurred.check.your.credentials"));
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div><div className="font-bold text-foreground text-sm">{i18n.t(isClient?"interface.client.space":"interface.technician.space")}</div><div className="text-xs text-muted-foreground">{i18n.t(mode==="login"?"auth.login":"interface.create.an.account")}</div></div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode==="register"&&<div className={`p-3 rounded-xl border-2 ${isClient?"border-blue-200 bg-blue-50/50":"border-emerald-200 bg-emerald-50/50"}`}><label className="block text-xs font-semibold mb-1.5">{i18n.t(isClient?"auth.cityRequired":"auth.businessCityRequired")}</label><input required minLength={2} maxLength={120} autoComplete="address-level2" placeholder={i18n.t("auth.cityExample")} value={form.city} onChange={(e)=>setForm((p)=>({...p,city:e.target.value}))} className={`w-full h-11 px-4 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none ${focus}`}/><p className={`mt-1.5 text-[11px] ${isClient?"text-blue-800":"text-emerald-800"}`}>{i18n.t("interface.required.this.location.is.used.by.default.when.gps.is.off")}</p></div>}
            <div><label className="block text-xs font-medium mb-1.5">Email</label><input type="email" required placeholder={i18n.t("common.emailPlaceholder")} value={form.email} onChange={(e)=>setForm((p)=>({...p,email:e.target.value}))} className={`w-full h-11 px-4 rounded-xl border border-gray-200 bg-gray-50 text-sm focus:outline-none ${focus}`}/></div>
            ...
            <button type="submit" disabled={loading} className={`w-full h-11 rounded-xl ${bg} text-white text-sm font-semibold mt-2 disabled:opacity-50`}>{i18n.t(loading?"interface.loading":mode==="login"?"interface.sign.in":"interface.create.my.account")}</button>
          </form>
          <div className="mt-5 text-center text-sm text-muted-foreground">{mode==="login"?<>{i18n.t("interface.no.account.yet")}{" "}<button onClick={()=>setMode("register")} className={`${cl} font-medium hover:underline`}>{i18n.t("interface.sign.up")}</button></>:<>{i18n.t("interface.already.have.an.account")}{" "}<button onClick={()=>setMode("login")} className={`${cl} font-medium hover:underline`}>{i18n.t("interface.sign.in")}</button></>}</div>
        </div>
      </div>
    </div>
  );
