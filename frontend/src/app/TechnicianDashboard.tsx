...
function TechTarifs({ city }: { city: string }) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6"><div><h2 className="text-xl font-bold" style={{ fontFamily:"Onest,sans-serif" }}>{t("Ma grille tarifaire","My pricing list")}</h2><p className="text-sm text-muted-foreground">{t("Devise déterminée par votre ville","Currency determined by your city")} : <strong>{profileCurrency}</strong>.</p></div><div className="flex flex-wrap justify-end gap-2"><button onClick={()=>setShowImport(!showImport)} className="flex items-center gap-2 h-9 px-4 rounded-lg border border-gray-200 bg-white text-sm font-semibold hover:bg-gray-50"><Upload className="w-4 h-4"/>{t("Importer un fichier","Import a file")}</button><button onClick={()=>setShowTariffDetails((visible)=>!visible)} className="flex items-center gap-2 h-9 px-4 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 text-sm font-semibold hover:bg-emerald-100">{showTariffDetails?t("Masquer les informations","Hide information"):t("Modifier les informations","Edit information")}</button></div></div>
      {showImport&&<div className="mb-6">
        <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${isDragActive?"border-emerald-400 bg-emerald-50":uploadStatus==="success"?"border-emerald-300 bg-emerald-50":uploadStatus==="error"?"border-red-300 bg-red-50":"border-gray-200 hover:border-emerald-300 bg-gray-50"}`}>
          <input {...getInputProps()}/>
          {uploadStatus==="idle"&&<><Upload className="w-8 h-8 mx-auto mb-3 text-muted-foreground"/><div className="text-sm font-medium mb-1">{isDragActive?t("Déposez ici","Drop here"):t("Importez votre grille tarifaire","Import your pricing list")}</div><div className="text-xs text-muted-foreground">{t("CSV, Excel .xlsx/.xlsm ou PDF texte — 5 Mo maximum","CSV, Excel .xlsx/.xlsm, or text PDF — 5 MB maximum")}</div></>}
      ...
      <div className="grid md:grid-cols-[380px_1fr] gap-6">
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3"><div><div className="text-sm font-semibold">{t("Horaires habituels","Regular working hours")}</div><div className="text-xs text-muted-foreground mt-0.5">{t("Votre semaine de travail récurrente","Your recurring work week")}</div></div><button onClick={()=>setShowHoursModal(true)} className="flex items-center gap-1.5 h-7 px-3 rounded-lg bg-emerald-50 text-emerald-700 text-xs hover:bg-emerald-100"><Clock className="w-3 h-3"/>{t("Modifier","Edit")}</button></div>
            <div className="space-y-1.5">{workingHours.map((item)=><div key={item.weekDay} className="flex items-center justify-between text-xs"><span className="text-slate-600">{weekDayNames[item.weekDay]}</span>{item.enabled?<span className="font-medium text-emerald-700">{item.startTime}–{item.endTime}</span>:<span className="text-muted-foreground">{t("Fermé","Closed")}</span>}</div>)}</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 space-y-2">{[["bg-emerald-100 border-emerald-400",t("Terminé","Completed")],["bg-blue-100 border-blue-400",t("Prévu","Scheduled")],["bg-gray-100 border-gray-300",t("Indisponible","Unavailable")]].map(([cls,l])=><div key={l} className="flex items-center gap-2 text-xs"><div className={`w-4 h-4 rounded border ${cls}`}/><span className="text-muted-foreground">{l}</span></div>)}</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3"><div><div className="text-sm font-semibold">{t("Exceptions et absences","Exceptions and time off")}</div><div className="text-xs text-muted-foreground mt-0.5">{t("Congés, formation ou absence ponctuelle","Leave, training, or one-off absence")}</div></div><button onClick={()=>setShowBlockModal(true)} className="flex items-center gap-1.5 h-7 px-3 rounded-lg bg-gray-100 text-xs hover:bg-gray-200"><Plus className="w-3 h-3"/>{t("Ajouter","Add")}</button></div>
            <div className="space-y-2">
              {blockedSlots.map((b)=><div key={b.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-gray-50 border border-gray-100"><BanIcon className="w-4 h-4 text-gray-400 shrink-0"/><div className="flex-1 min-w-0"><div className="text-xs font-medium truncate">{b.label}</div><div className="text-xs text-muted-foreground">{b.type==="daily"?`${t("Tous les jours","Every day")} ${b.startTime}–${b.endTime}`:b.type==="weekly"?`${b.weekDays?.map((d)=>weekDayNames[d]).join(", ")}`:b.date}</div></div><button onClick={()=>removeBlockedSlot(b.id)} className="text-muted-foreground hover:text-red-500"><X className="w-3.5 h-3.5"/></button></div>)}
              {blockedSlots.length===0&&<div className="text-xs text-muted-foreground text-center py-2">{t("Aucune indisponibilité","No unavailability")}</div>}
            </div>
          </div>
        </div>
        <div>
          <div className="mb-4"><h2 className="text-xl font-bold" style={{ fontFamily:"Onest,sans-serif" }}>{new Date(year,month,selectedDay).toLocaleDateString(locale,{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</h2><p className="text-sm text-muted-foreground">{dayApts.length} {t("rendez-vous","appointments")} · {hoursForDay(selectedDay)?.enabled?`${t("disponible","available")} ${hoursForDay(selectedDay)?.startTime}–${hoursForDay(selectedDay)?.endTime}`:t("journée non travaillée","non-working day")}</p></div>
          {blocksForDay(selectedDay).length>0&&<div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 flex items-start gap-3 text-sm text-amber-800"><BanIcon className="w-5 h-5 shrink-0"/><div><strong>{t("Créneaux bloqués","Blocked slots")} :</strong> {blocksForDay(selectedDay).map((block)=>`${block.startTime.slice(0,5)}–${block.endTime.slice(0,5)}`).join(", ")}. {t("Le reste de la journée demeure disponible.","The rest of the day remains available.")}</div></div>}
          {dayApts.length===0?<div className="bg-white rounded-xl border border-gray-100 p-12 text-center shadow-sm"><Calendar className="w-12 h-12 text-muted-foreground mx-auto mb-3 opacity-40"/><div className="text-sm text-muted-foreground">{t("Aucun rendez-vous ce jour","No appointments on this day")}</div></div>:(
