...
class AnthropicClient extends LlmClient {
  ...
  async generateJson({operation,system,payload,temperature}) {
    ...
    const response=await postJson(`${this.baseUrl}/messages`,{
      service:"llm",timeoutMs:this.timeoutMs,headers:{"x-api-key":this.apiKey,"anthropic-version":"2023-06-01"},
      body:{model:this.model,max_tokens:4096,system,messages:[{role:"user",content:JSON.stringify(payload)}],temperature},
    });
