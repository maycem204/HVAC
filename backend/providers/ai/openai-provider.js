...
class OpenAiClient extends LlmClient {
  ...
  async generateJson({ operation, system, payload }) {
    ...
    const response=await postJson(`${this.baseUrl}/responses`,{
      ...
      body:{model:this.model,instructions:system,input:JSON.stringify(payload),text:{format:{type:"json_object"}}},
    });
    const content=response?.output_text||response?.output?.flatMap((item)=>item.content||[]).find((item)=>item.type==="output_text")?.text;
